...
static void and_(bool canAssing);
...
static void dot(bool canAssign) {
  consume(TOKEN_IDENTIFIER, "Expect property named after '.'.");
...
static void expressionStatement() {
  expression();
  consume(TOKEN_SEMICOLON, "Expext ';' after expression.");
  emitByte(OP_POP);
}
...
static void printStatement() {
  expression();
  consume(TOKEN_SEMICOLON, "Expext ';' after value.");
  emitByte(OP_PRINT);
}
...
static void super_(bool canAssign) {
  if (currentClass == NULL) {
    error("Can't use 'super' outsde of a class.");
